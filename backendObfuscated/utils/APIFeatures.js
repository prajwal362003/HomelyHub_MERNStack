...
class APIFeatures {
  ...
  // Filter the query based on the queryString parameters
  filter() {
    let filterQuery = {};
    ...
    // Filtering based on room type
    if (queryObj.roomType) {
      let roomTypes = queryObj.roomType.split(",").map((type) => type.trim());
      filterQuery.roomType = { $in: roomTypes };
    }
  ...
  // Apply search based on city, state, or area
  search() {
    let searchQuery = {};
    let queryObj = { ...this.queryString };
